...
namespace Neral
{
    SoftRender::SoftRender(Ceiba::Window& window, const Vector4fAlignas16& position, const Vector4fAlignas16& gaze_direction, const Vector4fAlignas16& up_direction)
    ...
    {
        up_render_picture_ = std::make_unique<Gdiplus::Bitmap>(window.GetWidth(), window.GetHeight(), CEXPR_softrender_bitmap_pixel_format);
    ...
    Ceiba::EventState SoftRender::Rendering(const Ceiba::RenderEventArgs& event_args)
    {
        ...
        up_render_picture_->LockBits(&rect_to_lock, Gdiplus::ImageLockModeWrite, CEXPR_softrender_bitmap_pixel_format, &render_target_picture_data);
        //重置背景为黑色
    ...
    Ceiba::EventState SoftRender::OnWindowSizeChange(const Ceiba::SizeChangedEventArgs& event_args)
    {
        ...
        auto tmp_up = std::make_unique<Gdiplus::Bitmap>(event_args.new_width_, event_args.new_height_, CEXPR_softrender_bitmap_pixel_format);
        up_render_picture_.swap(tmp_up);
        return Ceiba::EventState::Continue;
    }

    SoftRender::DepthBuffer::DepthBuffer(Ceiba::Window& target_window, size_t width, size_t height)
